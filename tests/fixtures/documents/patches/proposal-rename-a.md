...
In the test configuration, the new scheduler completed the workload in 51 seconds. The current scheduler completed the same workload in 103 seconds.
...
The platform pod asked whether a new scheduler reduces completion time enough to justify the migration cost.

## System or method

The new scheduler assigns every runnable job at each cycle instead of one job.
...
Completion times were 49 to 54 seconds for the new scheduler. Completion times were 100 to 108 seconds for the current scheduler.

Peak memory was 1.8 gigabytes for the new scheduler. Peak memory was 1.5 gigabytes for the current scheduler.

## Interpretation

The evidence indicates a completion-time reduction for this workload. The new scheduler used about 20% more peak memory.
