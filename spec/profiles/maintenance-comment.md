...
| 4.13.10 | M | L | an anchor **resolves** when exactly one comment in the named source matches its comment hash. Zero matches = the comment is gone or edited and the record needs a second reading. Two or more matches = the anchor names no single comment. |
